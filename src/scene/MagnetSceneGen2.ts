...
export class MagnetScene {
  ...
  constructor(canvas: HTMLCanvasElement, callbacks: MagnetSceneCallbacks) {
    ...
    this.scene.background = new THREE.Color(0xe3e9ed);
    this.scene.fog = new THREE.Fog(0xe3e9ed, 30, 52);

    this.camera = new THREE.PerspectiveCamera(33, 1, 0.1, 100);
    this.camera.position.copy(cameraPresets.complete.position).multiplyScalar(1.13);
  ...
  private material(
    ...
    // Deterministic fine grain: no network textures or vendor assets.
    if (material.metalness > 0.65) {
      const size = 128;
      const pixels = new Uint8Array(size * size * 4);
      for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
        const value = 145 + Math.round(22 * Math.sin(y * 2.31) + 7 * Math.sin(x * 0.73 + y * 8.1));
        const i = (y * size + x) * 4;
      ...
      material.roughness = Math.max(material.roughness, 0.29);
  ...
  private buildLighting(): void {
    const hemisphere = new THREE.HemisphereLight(0xf4f8ff, 0x536171, 0.65);
    this.scene.add(hemisphere);

    const key = new THREE.DirectionalLight(0xfff8ef, 2.7);
    ...
    const rim = new THREE.DirectionalLight(0xf0f7ff, 2.8);
    rim.position.set(3, 6, -14);
    this.scene.add(rim);
  }

  private buildMagnet(): void {
    const bore = this.addLayerShell("bore-liner", 1, 0xe4e9e9, {
      metalness: 0.08,
      roughness: 0.2,
      clearcoat: 0.35,
    });
    ...
    const gradient = this.addLayerShell("gradient-assembly", 3, 0x476b80, {
      metalness: 0.18,
      roughness: 0.3,
      clearcoat: 0.6,
    ...
    const cryostat = this.addLayerShell("cryostat-inner", 4, 0x819896, {
      metalness: 0.36,
      roughness: 0.31,
      clearcoat: 0.34,
    });
  ...
  private addBoreNozzle(group: THREE.Group): void {
    const material = this.material(0xe4e9e9, {
      metalness: 0.08,
      roughness: 0.18,
      clearcoat: 0.38,
    });
    const nozzle = this.extrude(this.fullRingShape(1.98, 2.25), 0.72, FRONT_Z - 0.12, material, true);
  ...
  private buildOuterHousing(): void {
    ...
    for (const [parent, start, end] of [
      [closed, 0, Math.PI * 2], [window90, WINDOW_START, WINDOW_END],
      [reveal270, REVEAL_START, REVEAL_END],
    ] as const) {
      for (let angle = start + 0.09; angle < end - 0.04; angle += Math.PI / 18) {
        const bolt = new THREE.Mesh(new THREE.CylinderGeometry(0.065, 0.065, 0.055, 6), dark);
        bolt.rotation.x = Math.PI / 2;
        bolt.position.set(5.4 * Math.cos(angle), 5.4 * Math.sin(angle), FRONT_Z + 0.27);
  ...
  private fascia(material: SceneMaterial, start: number, arc: number): THREE.Group {
    // Revolved, gently crowned front cover with a rolled bore lip.
    const profile = [
      [2.08, -0.05], [2.08, 0.40], [2.10, 0.63], [2.20, 0.78],
      [2.40, 0.86], [3.0, 0.89], [4.2, 0.80], [5.45, 0.63],
      [6.10, 0.43], [6.34, 0.24], [6.43, 0.04], [6.40, -0.05], [2.08, -0.05],
    ].map(([r, z]) => new THREE.Vector2(r, z));
    const curve = new THREE.SplineCurve(profile);
    const geometry = new THREE.LatheGeometry(curve.getPoints(100), 160, start, arc);
    geometry.rotateX(Math.PI / 2);
    geometry.rotateZ(Math.PI / 2);
    geometry.translate(0, 0, FRONT_Z);
    ...
    if (arc < Math.PI * 2 - 0.01) {
      for (const angle of [start, start + arc]) {
        const cap = new THREE.ShapeGeometry(new THREE.Shape(curve.getPoints(100)));
        cap.rotateX(Math.PI / 2);
        cap.rotateZ(angle);
        cap.translate(0, 0, FRONT_Z);
        const edge = new THREE.Mesh(cap, material);
  ...
  private buildScannerCladding(): void {
    ...
    const window90 = new THREE.Group();
    const reveal270 = new THREE.Group();
    const white = this.material(0xf0f2f0, {
      metalness: 0.04,
      roughness: 0.28,
      clearcoat: 0.72,
    ...
    closed.add(
      ...
      this.fascia(white, 0, Math.PI * 2),
      this.extrude(this.fullRingShape(1.97, 2.12), 0.55, FRONT_Z + 0.22, trim, true),
    );
    const fasciaAccent = new THREE.Mesh(new THREE.TorusGeometry(5.94, 0.028, 12, 160), accent);
    fasciaAccent.position.z = FRONT_Z + 0.49;
    fasciaAccent.castShadow = true;

    const displayBezel = new THREE.Mesh(new RoundedBoxGeometry(1.12, 0.78, 0.17, 3, 0.07), trim);
    displayBezel.position.set(0, 4.48, FRONT_Z + 0.83);
    const displayFace = new THREE.Mesh(new THREE.BoxGeometry(0.82, 0.49, 0.045), screen);
    displayFace.position.set(0, 4.48, FRONT_Z + 0.94);
    closed.add(fasciaAccent, displayBezel, displayFace);

    [-1, 1].forEach((side) => {
      const panel = new THREE.Mesh(new RoundedBoxGeometry(0.72, 0.94, 0.16, 3, 0.06), trim);
      panel.position.set(side * 4.72, 0.15, FRONT_Z + 0.83);
      const face = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.69, 0.045), interfaceFace);
      face.position.set(side * 4.72, 0.15, FRONT_Z + 0.94);
    ...
    const glow = this.material(0xc7edf1, { metalness: 0.1, roughness: 0.24,
      emissive: new THREE.Color(0x5abac8), emissiveIntensity: 0.4 });
    const aperture = new THREE.Mesh(new THREE.TorusGeometry(2.18, 0.035, 12, 160), glow);
    aperture.position.z = FRONT_Z + 0.79;
    closed.add(aperture);
    // Recessed ventilation slots and fascia joints remain attached to cladding.
    for (const side of [-1, 1]) {
      for (let i = 0; i < 13; i++) {
        const slot = new THREE.Mesh(new RoundedBoxGeometry(0.035, 0.36, 0.025, 2, 0.01), dark);
        slot.position.set(side * (3.4 + i * 0.085), -3.5, FRONT_Z + 0.70);
        closed.add(slot);
      }
    }
    const addCutaway = (parent: THREE.Group, start: number, end: number) => {
      parent.add(
        ...
        this.fascia(white, start, end - start),
    ...
    const baseSkirt = new THREE.Mesh(new RoundedBoxGeometry(10.8, 1.25, 9.2, 4, 0.24), white);
    ...
    this.root.add(group);
    this.register(id, 8, group, [white, trim, dark, accent, screen, interfaceFace, glow]);
    this.cutawayVariants.set(id, { closed, window90, reveal270 });
  }
  ...
  private buildGround(): void {
    const ground = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xd5dde2, roughness: 0.83, metalness: 0.08 }),
    );
  ...
  private moveCamera(step: StepId, reducedMotion: boolean): void {
    ...
    const preset = { ...cameraPresets[step], position: cameraPresets[step].position.clone().multiplyScalar(1.13) };
    if (reducedMotion) {
